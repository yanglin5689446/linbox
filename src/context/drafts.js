...
export const DraftsWrapper = Component => props => {
  ...
  const newDraftEdit = useCallback(payload => dispatch({ type: actions.newDraftEdit, payload }), [])
  const closeDraftEdit = useCallback(payload => dispatch({ type: actions.closeDraftEdit, payload }), [])
